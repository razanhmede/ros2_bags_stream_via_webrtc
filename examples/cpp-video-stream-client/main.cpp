...
public:
  TopicVideoSource()
...

    {
      ...
      color_sub_ = create_subscription<sensor_msgs::msg::CompressedImage>(
          ...
          qos,
          std::bind(&TopicVideoSource::colorCompressedCallback, this, _1));
    }
    {
      rclcpp::QoS qos(rclcpp::KeepLast(5));
      qos.best_effort();
  ...
  void colorCompressedCallback(const sensor_msgs::msg::CompressedImage::SharedPtr msg) {
    try {
  ...
  void lidarRearCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    T_rear_to_front << -1.f, 0.f, 0.f, -1.365f,
                        -0.f, -1.f, 0.f,  0.245f,
                         0.f,  0.f, 1.f,  0.000f,
                         0.f,  0.f, 0.f,  1.000f;
...
private:
  void run() {
    ...
    while (rclcpp::ok() && !m_stopped) {
      ...
      cv::Mat rgb = color_image.clone();
      const cv::Size tile_sz = rgb.size();
      const int pad = 10;
      ...
      cv::copyMakeBorder(rgb, padded_color, 0, 0, 0, pad, cv::BORDER_CONSTANT);
      ...
      cv::Mat placeholder(rgb.rows, tile_sz.width + pad, CV_8UC3, cv::Scalar(255,255,255));


      cv::hconcat(std::vector<cv::Mat>{padded_color, placeholder}, top_row);
...
int main(int argc, char *argv[]) {
  ...
  const char *SIGNALING_URL = "ws://localhost:3001/signaling";
  std::vector<IceServer> iceServers = {IceServer(
      "turn:192.168.5.206:3478?transport=udp", "razan", "testrazan")};
